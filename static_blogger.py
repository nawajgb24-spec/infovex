...
def generate_automated_blog():
    try:
        ...
        search_prompt = f"Identify the absolute highest-traffic trending news topic or viral search query right now for '{chosen_category}' in 2026. Return ONLY the plain headline text without quotes."
        ...
        main_search_term = urllib.parse.quote(f"{chosen_category} tech")
...
def build_magazine_portal(db, latest_art, current_cat):
    def render_feed(cat_name):
        ...
        if not items:
            return "<p class='no-data'>AI scanning live feeds for upcoming updates...</p>"
    ...
    # Yahan humne saare CSS braces ko double {{ }} kar diya hai taaki python variable error na aaye
...
<head>
    ...
    <style>
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; }}
        ...
        header {{ background-color: rgba(6, 9, 19, 0.9); backdrop-filter: blur(20px); border-bottom: 1px solid var(--border-glass); position: sticky; top: 0; z-index: 2000; }}
        .header-wrap {{ max-width: 1400px; margin: 0 auto; display: flex; justify-content: space-between; align-items: center; padding: 18px 24px; }}
        ...
        .portal-grid {{ max-width: 1400px; margin: 32px auto; display: grid; grid-template-columns: 2.3fr 1fr; gap: 36px; padding: 0 24px; }}
        ...
        .short-premium-card {{ position: relative; width: 100%; height: 190px; border-radius: 14px; overflow: hidden; margin-bottom: 16px; border: 1px solid var(--border-glass); box-shadow: 0 4px 20px rgba(0,0,0,0.5); transition: transform 0.2s ease, border-color 0.2s ease; }}
        ...
        .short-card-overlay {{ position: absolute; bottom: 0; left: 0; width: 100%; padding: 16px; background: linear-gradient(0deg, rgba(6,9,19,1) 0%, rgba(6,9,19,0.4) 70%, rgba(6,9,19,0) 100%); display: flex; flex-direction: column; justify-content: flex-end; height: 100%; }}
        .short-tag {{ background: var(--hyper-blue); color: #060913; font-size: 9px; font-weight: 800; padding: 2px 6px; border-radius: 4px; text-transform: uppercase; width: fit-content; margin-bottom: 8px; letter-spacing: 0.5px; }}
        ...
        @media(max-width: 1024px) {{ .portal-grid {{ grid-template-columns: 1fr; gap: 40px; }} .sidebar-shorts {{ position: static; }} }}
        @media(max-width: 640px) {{ .header-wrap {{ padding: 16px; }} .brand {{ font-size: 23px; }} h1 {{ font-size: 26px; }} .spotlight-hero img {{ height: 240px; }} }}
    </style>
</head>
<body>
<header>
...
<main class="portal-grid">
    <section>
        <div class="spotlight-hero">
            <img src="{latest_art['image']}" alt="Spotlight">
        ...
        <div class="editorial-row"><div class="row-heading">🔌 Electronics & Hardware</div><div class="cards-flex-grid">{render_feed("Electronics")}</div></div>
        <div class="editorial-row"><div class="row-heading">🏆 Global Sports Track</div><div class="cards-flex-grid">{render_feed("Sports")}</div></div>
        <div class="editorial-row"><div class="row-heading">💡 Interesting Facts Hub</div><div class="cards-flex-grid">{render_feed("Interesting Facts")}</div></div>
        <div class="editorial-row"><div class="row-heading">🎬 Global Film Industry</div><div class="cards-flex-grid">{render_feed("Film Industry")}</div></div>
        <div class="editorial-row"><div class="row-heading">🌱 Health & Food Insights</div><div class="cards-flex-grid">{render_feed("Health & Food")}</div></div>
    </section>
    ...
    with open("index.html", "w", encoding="utf-8") as f:
        f.write(full_html)
    print("Code compiling completed successfully.")
